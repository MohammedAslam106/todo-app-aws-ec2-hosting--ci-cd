    ...
    return (
        <div className=" w-full h-screen flex justify-center items-center">
            <div className=" w-full flex flex-col justify-between gap-5 items-center px-4 py-4 bg-black/50 shadow-md rounded-xl max-w-md mx-auto ">
                <h1 className="text-md font-bold mb-0">Signup</h1>

                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full h-full max-w-sm mt-5">
                    <input type="text" placeholder="Name" className="w-full px-4 py-2 rounded-md bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" {...register("name", { required: true })} />
                    ...
                    <button type="submit" className="w-full px-4 py-2 rounded-md bg-blue-500 text-white hover:bg-blue-600">Signup</button>

                    <Link to='/login' className="text-sm text-gray-400 hover:text-gray-200 mt-2">Already have an account? Login</Link>
                </form>
            </div>
        </div>
    )
